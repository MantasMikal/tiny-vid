...
use tiny_vid_core::test_support::VideoMetadataForTest;

fn assert_basic_video_metadata(metadata: &VideoMetadataForTest) {
    assert!(
...
fn command_get_video_metadata_returns_metadata_for_video() {
    ...
    let metadata = tiny_vid_core::test_support::media_inspect_metadata_for_test(input_path)
        .expect("media_inspect_metadata_for_test");

    assert_basic_video_metadata(&metadata);
}
...
fn command_get_build_variant_returns_known_codecs() {
    assert_codec_contract(CodecContract::IntegrationSmoke);
    let variant = tiny_vid_core::test_support::app_capabilities_for_test()
        .expect("app_capabilities_for_test");
    assert_eq!(variant.protocol_version, 2);
