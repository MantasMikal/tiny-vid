...
/// Returns true when preview original segment extraction can stream-copy to MP4 and still
...
pub fn is_preview_stream_copy_safe_codec(codec_name: &str) -> bool {
    ...
    #[cfg(target_os = "linux")]
    {
        return matches!(
            lower.as_str(),
            "h264" | "avc" | "avc1" | "vp9" | "vp09" | "av1" | "av01"
        );
    }
...
mod tests {
    ...
    #[test]
    fn videotoolbox_quality_extremes_map_to_qv() {
        for (quality, expected_qv) in [(0_u32, "0"), (100_u32, "100")] {
            let mut o = opts();
            o.codec = Some("h264_videotoolbox".to_string());
            o.quality = Some(quality);
            let args = build_ffmpeg_command("/in.mp4", "/out.mp4", &o, None, None, None).unwrap();
            let qv_idx = args.iter().position(|a| a == "-q:v").unwrap();
            assert_eq!(
    ...
    fn preview_stream_copy_safe_codecs_match_linux_allow_list() {
        ...
        assert!(is_preview_stream_copy_safe_codec("vp09"));
        assert!(is_preview_stream_copy_safe_codec("av1"));
        assert!(is_preview_stream_copy_safe_codec("av01"));
        assert!(!is_preview_stream_copy_safe_codec("hevc"));
        assert!(!is_preview_stream_copy_safe_codec("h265"));
        assert!(!is_preview_stream_copy_safe_codec("hvc1"));
        assert!(!is_preview_stream_copy_safe_codec("hev1"));
    }
