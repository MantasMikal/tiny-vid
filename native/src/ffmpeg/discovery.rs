...
#[cfg(any(target_os = "macos", target_os = "windows"))]
fn bundled_sidecar_base_names() -> Vec<String> {
    let mut names = vec![format!("ffmpeg-{}", bundled_target_suffix())];
    names.push("ffmpeg".to_string());
    names
}

#[cfg(target_os = "macos")]
fn bundled_target_suffix() -> String {
...
fn bundled_target_suffix() -> String {
    format!("{}-pc-windows-gnu", std::env::consts::ARCH)
}
