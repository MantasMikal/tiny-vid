...
const SOFTWARE_PREFERRED_OVER_VT: &[&str] = &["libx264", "libx265"];
...
fn sort_codecs_for_display(codecs: &[String]) -> Vec<String> {
    let mut sorted = codecs.to_vec();
    sorted.sort_by_key(|codec| {
        DISPLAY_ORDER
            .iter()
            .position(|candidate| *candidate == codec.as_str())
            .unwrap_or(usize::MAX)
    });
    sorted
}

/// Hide VideoToolbox only when x264/x265 is available; otherwise keep VT visible.
/// This preserves VT in the LGPL profile (VT + AV1/VP9) while still preferring x264/x265 in GPL builds.
pub fn filter_codecs_for_display(available: &[String]) -> Vec<String> {
    let has_preferred_software = available
        .iter()
        .any(|c| SOFTWARE_PREFERRED_OVER_VT.contains(&c.as_str()));
    let filtered = if has_preferred_software {
        available
            .iter()
    ...
    sort_codecs_for_display(&filtered)
}
...
mod tests {
    ...
    #[test]
    fn filter_codecs_hides_videotoolbox_when_x264_or_x265_available() {
        let available = vec![
            "libx264".to_string(),
    ...
    #[test]
    fn filter_codecs_keeps_videotoolbox_with_lgpl_software_codecs() {
        let available = vec![
            "libsvtav1".to_string(),
