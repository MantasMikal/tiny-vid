...
import { FFMPEG_PROFILES } from "./ffmpeg-profile.ts";
import { BUILD_MODES, BUILD_PLATFORMS, TEST_SUITES } from "./standalone.ts";

export function dryRunOption(): Option {
  return new Option("--dry-run", "Print commands and environment overrides without executing them");
}
...
export function profileOption(required = false): Option {
  const option = new Option("--profile <profile>").choices(FFMPEG_PROFILES);
  if (required) {
