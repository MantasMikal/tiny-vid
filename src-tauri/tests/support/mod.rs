...
pub fn assert_codec_contract(contract: CodecContract) {
    let ffmpeg_path =
        tiny_vid_tauri_lib::ffmpeg::discovery::get_ffmpeg_path().unwrap_or_else(|e| {
            panic!(
...
fn required_codecs(contract: CodecContract) -> Vec<&'static str> {
    #[cfg(feature = "lgpl")]
    {
        match contract {
            ...
            CodecContract::IntegrationContract => vec!["h264_videotoolbox", "hevc_videotoolbox"],
        }
    }
...
impl IntegrationEnv {
    pub fn new() -> Self {
        let ffmpeg = tiny_vid_tauri_lib::ffmpeg::discovery::get_ffmpeg_path()
...
pub fn run_transcode_and_verify(
    ...
    run_ffmpeg_blocking(args, None, None, duration_secs, None, None)
...
pub fn run_preview_with_estimate_and_assert(
    ...
    options: &TranscodeOptions,
    preview_start_seconds: Option<f64>,
) -> tiny_vid_tauri_lib::test_support::PreviewWithEstimateResultForTest {
    let result = tauri::async_runtime::block_on(
        tiny_vid_tauri_lib::test_support::run_preview_with_estimate_for_test(
            input_path,
            options,
            preview_start_seconds,
        ),
...
pub fn metadata(path: &Path) -> tiny_vid_tauri_lib::ffmpeg::ffprobe::VideoMetadata {
